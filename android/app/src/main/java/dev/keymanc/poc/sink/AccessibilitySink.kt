...
import dev.keymanc.poc.input.Cursor
import dev.keymanc.poc.input.HidKeycodes
import dev.keymanc.poc.ui.CursorOverlay
 ...
 * keymanc IME when it is the active keyboard.
 */
class AccessibilitySink : InputSink {
    private var service: KeymancAccessibilityService? = null
    ...
    override fun start(): String? {
        val s = KeymancAccessibilityService.instance
            ?: return "접근성 서비스 'keymanc'를 켜 주세요 (설정 → 접근성)"
        service = s
        overlay = CursorOverlay(s, WindowManager.LayoutParams.TYPE_ACCESSIBILITY_OVERLAY)
        if (KeymancIme.instance == null) AppLog.i("keymanc IME is not the active keyboard: only Esc/Win keys will work")
        return null
    }
    ...
    private fun endPress(s: KeymancAccessibilityService) {
        val p = path ?: return
        path = null
    ...
    private fun fallbackKey(code: Int, down: Boolean) {
        ...
        when (code) {
            ...
            KeyEvent.KEYCODE_META_LEFT, KeyEvent.KEYCODE_META_RIGHT -> s.performGlobalAction(AccessibilityService.GLOBAL_ACTION_HOME)
                ...
                warnedNoIme = true
                AppLog.i("key dropped: select 'keymanc' as the keyboard to type")
            }
        }
    }
