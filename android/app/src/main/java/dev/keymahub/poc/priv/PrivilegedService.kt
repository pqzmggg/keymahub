...
import dev.keymahub.poc.host.EvdevCapture
    ...
    override fun onTransact(code: Int, data: Parcel, reply: Parcel?, flags: Int): Boolean {
        ...
        when (code) {
            ...
            PrivProtocol.UHID_CREATE -> {
                val err = runCatching {
                    destroyUhid()
                    keyboard = UhidDevice("keymahub Keyboard", HidDescriptors.KEYBOARD, VENDOR, PRODUCT_KEYBOARD)
                    mouse = UhidDevice("keymahub Mouse", HidDescriptors.MOUSE, VENDOR, PRODUCT_MOUSE)
                }.exceptionOrNull()
                reply?.writeNoException()
