...
import dev.keymahub.poc.MainActivity
import dev.keymahub.poc.a11y.KeymahubAccessibilityService
import dev.keymahub.poc.Prefs
import dev.keymahub.poc.bt.HidTransport
import dev.keymahub.poc.bt.BtDiagnostics
import dev.keymahub.poc.bt.BtHidSink
...
class HostService : Service() {
    ...
    private fun startA11yCapture(s: BtHidSink): String? {
        val service = KeymahubAccessibilityService.instance
            ?: return "keymahub 접근성 서비스가 꺼져 있습니다 (설정 → 접근성 → keymahub 켜기, Shizuku 불필요)"
    ...
    private fun stopCapture() {
        ...
        if (capture != null) {
            pointerCapture?.stop()
            pointerCapture = null
            KeymahubAccessibilityService.instance?.let { if (it.hostCapture === capture) it.hostCapture = null }
            capture.stop()
            a11y = null
        } else {
    ...
    private fun onFocus(remote: Boolean) {
        ...
        if (a11y != null) {
            if (remote) {
                ...
                pointerCapture?.stop()
                KeymahubAccessibilityService.instance?.interceptMouse(false)
            }
        }
    ...
    private fun notification(): Notification {
        ...
        return Notification.Builder(this, CHANNEL)
            ...
            .setContentTitle(if (remoteFocus) "keymahub 호스트 — 대상 기기 조작 중" else "keymahub 호스트 — 이 폰 조작 중")
    ...
    private fun startInForeground() {
        getSystemService(NotificationManager::class.java)!!
            .createNotificationChannel(NotificationChannel(CHANNEL, "keymahub host", NotificationManager.IMPORTANCE_LOW))
    ...
    companion object {
        ...
        private const val ACTION_STOP = "dev.keymahub.poc.HOST_STOP"
